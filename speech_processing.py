    ...
    try:
        ...
        while True:
            ...
            if rec.AcceptWaveform(data):
                ...
                if "result" in result:
                    transcript_data.extend(result["result"])
        ...
        # ✅ Return word-level segments for scene-based summaries
        segments = [
            {"start": entry["start"], "end": entry["end"], "text": entry["word"]}
            for entry in transcript_data
        ]

        return transcript.strip(), audio_path, srt_file_path, segments
